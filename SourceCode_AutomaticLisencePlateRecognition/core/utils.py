...
#kode di bawah berfungsi untuk load tesseractOCR, jika tidak digunakan dapat dihapus atau dimatikan
pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
# Example tesseract_cmd = r'C:\Program Files (x86)\Tesseract-OCR\tesseract'

'''
#percobaan recognize plate tesseract, proses dimodifikasi
#function recognize_plate di bawah ini yang error di bagian openCV, sementara masih blm nemu solusi
def recognize_plate(img, coords):
    ...
    #rect_kern = cv2.getStructuringElement(cv2.MORPH_RECT, (18,18))

    #rect_kern = cv2.getStructuringElement(cv2.MORPH_CROSS, (5,5))
    ...
    for cnt in sorted_contours:
        ...
        try:
            ...
            text = None
    if plate_num != None:
        print("PLAT NOMOR #: ", plate_num)
    else:
        print("PLAT NOMOR #: Not Found")
...
#function recognize_plate di bawah ini menggunakan EasyOCR, dapat berjalan tanpa error
def recognize_plate(img, coords):
    ...
    #cv2.waitKey(0)
    #text = easyocr.Reader(['en'], gpu= False).readtext(box, allowlist='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ', width_ths=5, detail=0, batch_size=10, min_size=20)

    text = easyocr.Reader(['en'], gpu= False).readtext(box, allowlist='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 ', width_ths=5, detail=0, batch_size=10)
    
    text_hitung = len(text)
    #print(text_hitung)
    #print("PLAT NOMOR #: ", text)
    
    if text_hitung == 0:
        print("TNKB         #: Not Found")
        print("masa berlaku #: Not Found")
...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            if info:
                print("Object found: {}, Confidence: {:.2f}, BBox Coords (xmin, ymin, xmax, ymax): {}, {}, {}, {} ".format(class_name, score, int(coor[0]), coor[1], coor[2], coor[3]))
